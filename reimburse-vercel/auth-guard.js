...
(function () {
  ...
  window.renderUserbar = function (me) {
    var el = document.getElementById('rmb-userbar'); if(!el) return;
    ...
    var nav = [ navlink('/','New Claim', p==='/'||p==='/index.html'), navlink('/my','My Claims', p.indexOf('/my')>-1) ];
    if (isFin) {
      ...
      if (isCEO) nav.push(navlink('/approvals','CEO', onAppr));
    }
  ...
  function denied(me, role){
    ...
    var links = [['/','New Claim', true],['/my','My Claims', true]];
