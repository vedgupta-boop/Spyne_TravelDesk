...
import { forexData, saveForexConfirm, completeForex, saveForexTopup, loadForexTopup, recallRequest } from '../lib/workflow.js';
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      if (b.action === 'topup-load') { const s = getSession(req); res.status(200).json(await loadForexTopup({ id: b.id, idx: b.idx, email: s && s.email, roles: (s && s.roles) || [] }, baseUrl(req))); return; }
