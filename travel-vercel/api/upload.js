...
import { parseTicket } from '../lib/ticketparse.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const out = { ok: true, link: r.link, id: r.id };
    // Admin booking flow: auto-extract ticket fields from the uploaded flight ticket (best-effort).
    if (b.parseTicket) {
      const parsed = await parseTicket({ base64: b.base64, mimeType: b.mimeType });
      if (parsed && parsed.ok) out.extracted = parsed.fields;
    }
