...
// Roles are RE-DERIVED from the signed email on every read (not trusted from the cookie), so a
// role reassignment takes effect immediately — as long as applyRoleOverrides() ran earlier in the
// request to refresh CONFIG. Falls back to the cookie's roles if re-derivation isn't possible.
export function getSession(req) {
  const token = parseCookies(req)[SESSION_COOKIE];
  const s = readSessionToken(token); // { email, roles, name, exp } | null
  if (s && s.email) { try { s.roles = rolesFor(s.email); } catch { /* keep cookie roles */ } }
  return s;
}

// Guard a data API. Returns the session, or null after sending a 401/403.
// Finance is a superuser: it can access every dashboard/API (finance, admin, forex).
